...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		ormma.addEventListener('viewableChange', function(viewable) {
			showit('Received viewableChange event...');
			showit('ormma.getViewable() is : ' + viewable);
		});
		setTimeout(function() {
			showit('Calling ormma.getViewable()...');
			showit('ormma.getViewable() is : ' + ormma.getViewable());
		}, 100);
