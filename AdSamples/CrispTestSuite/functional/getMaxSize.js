...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		var ormmaMethods = ['getMaxSize', 'getScreenSize', 'getDefaultPosition'];
		var hasOrmmaMethods;
		var hasError = false;
		...
		setTimeout(function() {
			showit('calling ormma.getMaxSize()...');
			showit('ormma.getMaxSize().width is : ' + ormma.getMaxSize().width);
			showit('ormma.getMaxSize().height is : ' + ormma.getMaxSize().height);

			showit('calling ormma.getScreenSize()...');
			showit('ormma.getScreenSize().width is : ' + ormma.getScreenSize().width);
			showit('ormma.getScreenSize().height is : ' + ormma.getScreenSize().height);

			showit('calling ormma.getDefaultPosition()...');
			showit('ormma.getDefaultPosition().x is : ' + ormma.getDefaultPosition().x);
			showit('ormma.getDefaultPosition().y is : ' + ormma.getDefaultPosition().y);
		}, 100);
