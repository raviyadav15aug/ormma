...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		setTimeout(function() {
			showit('calling ormma.request()...');
			var uri = 'http://demo.crispadvertising.com/app/img/iPhoneAd/QA/320x460_panel.JPG';
			ormma.addEventListener('response', function(uri, response) {
				showit('Response received: ' + response);
			});
			var bool = ormma.request(uri, 'ignore');
			setTimeout(function() {
				showit('ormma.request() correctly returned false'  (bool == false ? ' - passed.' : ' - failed.'));
			}, 10);
		}, 100);
