...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		showit('Listening to headingChange event...');
		ormma.addEventListener('headingChange', function(width, height) {
			showit('Received sizeChange event...');
			showit('ormma.getHeading() is : ' + ormma.getHeading() + (ormma.getHeading() === -1 ? ' (unknown)' : ''));
		});
		setTimeout(function() {
			showit('Calling ormma.getHeading()...');
			showit('ormma.getHeading() is : ' + ormma.getHeading() + (ormma.getHeading() === -1 ? ' (unknown)' : ''));
		}, 100);
