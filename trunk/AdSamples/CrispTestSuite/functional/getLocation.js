...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		showit('Listening to locationChange event...');
		ormma.addEventListener('locationChange', function(width, height) {
			setTimeout(function() {
				showit('');
				showit('Received locationChange event...');
				showit('ormma.getLocation().lat is : ' + ormma.getLocation().lat);
				showit('ormma.getLocation().lon is : ' + ormma.getLocation().lon);
				showit('ormma.getLocation().acc is : ' + ormma.getLocation().acc);
			}, 100);
		});
		setTimeout(function() {
			showit('');
