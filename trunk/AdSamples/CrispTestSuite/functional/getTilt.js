...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		ormma.addEventListener('tileChange', function(x, y, z) {
			showit('Received tileChange event...');
			showit('x: ' + x + 'x: ' + y + 'z: ' + z);
		});
		setTimeout(function() {
			showit('calling ormma.getTilt()...');
			var tilt = ormma.getTilt();
			if (tilt) {
				showit('ormma.getTilt().x is : ' + tilt.x);
				showit('ormma.getTilt().y is : ' + tilt.y);
				showit('ormma.getTilt().z is : ' + tilt.z);
			} else {
				showit('ormma.getTilt() does not have a return value - failed');
			}
		}, 100);
