...
function ORMMAReady(evt) {
	...
	if (window.ormmaAvail !== ORMMAReadyStatus.NOT_FOUND) {
		...
		if (!hasError) {
			ormma.addEventListener('keyboardChange', function(width, height) {
				showit('Received keyboardChange event...');
				showit('ormma.getKeyboard() is : ' + ormma.getKeyboard());
			});
			setTimeout(function() {
				showit('calling ormma.getKeyboard()...');
				showit('ormma.getKeyboard() is : ' + ormma.getKeyboard());
			}, 100);
		}
	}
